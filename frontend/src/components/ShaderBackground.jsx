...
function ShaderPlane() {
  ...
  useFrame((state) => {
    ...
    // Zoom to cover: scale plane so it always fills the viewport (like background-size: cover)
    const aspect = width / height;
    const d = 1.5;
    const vFov = (75 * Math.PI) / 180;
    const halfHeight = d * Math.tan(vFov / 2);
    const halfWidth = halfHeight * aspect;
    const scaleX = (2 * halfWidth) / 4;
    const scaleY = (2 * halfHeight) / 4;
    const coverScale = Math.max(scaleX, scaleY) * 1.05; // max = zoom in to fill; 5% overflow
    meshRef.current.scale.x = coverScale;
    meshRef.current.scale.y = coverScale;
  });
...
function ShaderBackground() {
  ...
  return (
    <div
      className="shader-bg fixed inset-0 -z-10 min-h-screen w-screen overflow-hidden"
      style={{
        // Full-bleed: no margins/padding; background only for pre-canvas flash
    ...
    >
      ...
      {/* Overlay: no dark at top so purple is full-bleed from y=0; only subtle bottom vignette */}
      <div
        style={{
          pointerEvents: 'none',
          position: 'absolute',
